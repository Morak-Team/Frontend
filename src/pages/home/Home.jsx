import { Link } from "react-router-dom";
import { usePaymentStore } from "@/store/paymentStore";

const Home = () => {
  const { reviewInfo } = usePaymentStore();
  console.log(reviewInfo);
  return (
    <div className="p-6 space-y-4">
      <h2 className="text-xl">모락의 Home page입니다.</h2>

      {/* 모바일 웹 테스트 용 버튼 */}
      <div className="flex flex-col space-y-3 mt-6">
        <Link
          to="/auth"
          className="px-4 py-2 bg-yellow-400 rounded-lg text-center text-white font-semibold shadow-md hover:bg-yellow-500 transition"
        >
          로그인 페이지로 이동
        </Link>

        <Link
          to="/map"
          className="px-4 py-2 bg-green-500 rounded-lg text-center text-white font-semibold shadow-md hover:bg-green-600 transition"
        >
          지도 페이지로 이동
        </Link>

        <Link
          to="/myPage"
          className="px-4 py-2 bg-pink-300 rounded-lg text-center text-white font-semibold shadow-md hover:bg-green-600 transition"
        >
          마이 페이지로 이동
        </Link>
        <Link
          to="/signup"
          className="px-4 py-2 bg-secondary3 rounded-lg text-center text-white font-semibold shadow-md hover:bg-green-600 transition"
        >
          회원가입 페이지로 이동
        </Link>
      </div>
    </div>
  );
};

export default Home;
